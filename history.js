...
async function fetchTrackInfo(trackId) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      elementTwo.style.backgroundImage = `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('${albumImage}')`;
      const elementThree = document.querySelector('#home');
      elementThree.style.backgroundImage = `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('${albumImage}')`;
      const elementFour =
        document.querySelector('#library');
      elementFour.style.backgroundImage = `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('${albumImage}')`;
      ...
      if (previewUrl) {
        ...
        // Defualt volume drag bar not following, needs fix
